...
def model_dim_fn(cuda):
    ...
    #resume = os.path.join(os.path.dirname(os.path.realpath(__file__)), '../stage1_sad_54.4.pth')
    model = VGG16(stage)
    if cuda:
...
def matting_result(pk_o, pic_input, tri_input, title, model, cuda, website = False):
    ...
    img_mat = Image.fromarray(im)
    out_m_name = title.split('_mask')[0]+'_matting.png'
    img_io = BytesIO()
    img_mat.save(img_io, format='JPEG')
